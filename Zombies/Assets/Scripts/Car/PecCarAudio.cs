...
namespace pec4.car
{
	public class PecCarAudio : MonoBehaviour
	{
		...
		private PecCarController m_CarController; // Reference to car we are controlling
		private bool useSound = false;

		private void StartSound()
		{
			// get the carcontroller ( this will not be null as we have require component)
			m_CarController = GetComponent<PecCarController>();
		...
		public void UseSound(bool useSound)
		{
		...
		private void Update()
		{
			...
			if (m_StartedSound)
			{
				...
				else
				{
					...
					m_LowAccel.volume = (!useSound) ? 0 : lowFade * accFade;
					m_LowDecel.volume = (!useSound) ? 0 : lowFade * decFade;
					m_HighAccel.volume = (!useSound) ? 0 : highFade * accFade;
					m_HighDecel.volume = (!useSound) ? 0 : highFade * decFade;
